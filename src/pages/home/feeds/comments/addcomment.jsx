...
import { View, TextInput, TouchableOpacity, Image, StyleSheet, Platform, Keyboard, Alert } from 'react-native';
...
const AddComment = forwardRef(({ user, feedId, token }, ref) => {
    const [commentText, setCommentText] = useState('');
    const [posting, setPosting] = useState(false);
    const [keyboardHeight, setKeyboardHeight] = useState(0);
    const inputRef = useRef(null);

    // Forward the ref so parent can call .focus()
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            if (response && response.status === 200) {
                // Comment posted successfully — CommentBonds will refresh via its own polling
            } else {
            ...
            <View style={styles.inputContainer}>
                ...
                <TouchableOpacity style={styles.postButtonContainer} onPress={handlePost}>
                    <Ionicons
                        name="send"
                        size={20}
                        color={commentText ? (AppDetails.primaryColor || '#0095f6') : '#ccc'}
                    />
                </TouchableOpacity>
            </View>
        </View>
